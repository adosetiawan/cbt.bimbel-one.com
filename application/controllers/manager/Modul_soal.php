...
class Modul_soal extends Member_Controller {
    ...
    function tambah(){
		...
		if($this->input->post('tambah-tipe', TRUE) != 4){
			$this->form_validation->set_rules('tambah-soal', 'Soal','required');
		}else{
			$this->form_validation->set_rules('cermat-A', 'Soal Cermat A','required');
			$this->form_validation->set_rules('cermat-B', 'Soal Cermat B','required');
        ...
        if($this->form_validation->run() == TRUE){
        	...
        	if($id_topik=='kosong'){
	        	...
	        	$data['soal_cermat_a'] = $this->input->post('cermat-A', TRUE);
	        	$data['soal_cermat_b'] = $this->input->post('cermat-B', TRUE);
