...
class Modul_jawaban extends Member_Controller {
        ...
        if($id_soal!=null){
        	...
        	if($query_soal->num_rows()>0){
        		...
        		if($query_soal->soal_tipe!=2){
        			...
        			$data['tipe_soal'] = $query_soal->soal_tipe;
						...
						$soal = '<table border="1" cellspacing="0" style="width:530.9pt">
									<tbody>
										...
										<tr>
											...
											<h1 style="text-align: center;"><strong>'.$query_soal->soal_cermat_a.'</strong></h1>
											</td>
											<td style="width:18.7pt">
											<h1 style="text-align: center;"><strong>'.$query_soal->soal_cermat_b.'</strong></h1>
											</td>
											<td style="width:18.7pt">
											<h1 style="text-align: center;"><strong>'.$query_soal->soal_cermat_c.'</strong></h1>
											</td>
											<td style="width:18.7pt">
											<h1 style="text-align: center;"><strong>'.$query_soal->soal_cermat_d.'</strong></h1>
											</td>
											<td style="width:18.7pt">
											<h1 style="text-align: center;"><strong>'.$query_soal->soal_cermat_e.'</strong></h1>
											</td>
										</tr>
										<tr>
											<td style="width:18.7pt">
    ...
    function tambah_kecermatan(){
    	// $this->load->library('form_validation');
        
        // $this->form_validation->set_rules('jawaban-a', 'Jawaban A','required|strip_tags');
        // $this->form_validation->set_rules('jawaban-b', 'Jawaban B','required|strip_tags');
        // $this->form_validation->set_rules('jawaban-c', 'Jawaban C','required|strip_tags');
        // $this->form_validation->set_rules('jawaban-d', 'Jawaban D','required|strip_tags');
        ...
        // if($this->form_validation->run() == TRUE){
	        ...
	        if(!empty($id_jawaban)){
	        	 ...
	        	 * Jika jawaban baru
	        	 */	
				$jawabanA =  $this->input->post('jawaban-a[]');
				$jawabanB =  $this->input->post('jawaban-b[]');
				$jawabanC =  $this->input->post('jawaban-c[]');
				$jawabanD =  $this->input->post('jawaban-d[]');

				if(count($jawabanA)>0){
					foreach($jawabanA as $id => $value){
						if(!empty($value)&&!empty($jawabanB[$id])&&!empty($jawabanC[$id])&&!empty($jawabanD[$id])){
							$data['jawaban_soal_id'] = $id_soal;	
							$data['jawaban_cermat_1'] = $value;	
							$data['jawaban_cermat_2'] = $jawabanB[$id];	
							$data['jawaban_cermat_3'] = $jawabanC[$id];	
							$data['jawaban_cermat_4'] = $jawabanD[$id];	
							$data['jawaban_benar'] = 1;	
    ...
    function get_datatable(){
		...
		$querySoal = $this->db->get_where('cbt_soal',['soal_id'=>$soal]);
		if($querySoal->num_rows()>0){
			$tipeSoal = $querySoal->row()->soal_tipe;
		}else{
			$tipeSoal = 0;
		}
	    ...
	    foreach ($query as $temp) {			
			...
			if($tipeSoal == 4){
				$record[] = '<table  border="1"> 
									<tr>
									<td style="width:50px;text-align:center">'.$temp->jawaban_cermat_1.'</td>
									<td style="width:50px;text-align:center">'.$temp->jawaban_cermat_2.'</td>
									<td style="width:50px;text-align:center">'.$temp->jawaban_cermat_3.'</td>
									<td style="width:50px;text-align:center">'.$temp->jawaban_cermat_4.'</td>
								</tr>
							</table>';
			}else{
